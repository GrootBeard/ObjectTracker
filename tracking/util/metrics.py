...
class PoissonClutter:
    __slots__ = ("rng", "x_bounds", "y_bounds",
                 "clutter_density", "width", "height")
...
class RadarGenerator:
    ...
    def make_scans_series(self, probes, clutter_generator=None):
        ...
        for scan_id, probe in enumerate(probes):
            ...
            if measurements:
                scans.append(
                    Scan(time=probe[0], measurements=measurements, scan_id=scan_id))
...
class TrackLog:
    ...
    def add_entry(self, x, P, time: float, type: LogEntryType, considered_measurements: list[Measurement]) -> None:
        ...
        self.entries.append(LogEntry(x=xval, P=Pval, time=time,
                            type=type, considered_measurements=considered_measurements))
