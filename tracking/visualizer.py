...
class TrackVisualizer:
    ...
    def _buffer_track_epoch(self, track: int, epoch: int):
        ...
        for ep in epoch_predictions[:-1]:
            self._prediction_buffer += [[self.predictions[track][ep-epoch, 0], self.predictions[track][ep-epoch, 1]]]
    ...
    def _render_track_redline(self, plot: plt.Axes, track: int) -> None:
        ...
        plot.plot(self.redline[track][:pos, 0], self.redline[track][:pos,1], color='red', linewidth=0.7)
